...
def null_dept
  ...
  FROM teachers
  FULL OUTER JOIN depts ON dept_id = depts.id
...
def all_depts_join
  ...
  SELECT teachers.name, depts.name
  FROM teachers
  RIGHT JOIN depts ON dept_id = depts.id
  SQL
end
...
def teachers_and_divisions
  ...
  WHEN dept_id IN (1, 2) THEN 'Sci' ELSE 'Art' END
  FROM teachers
  SQL
end
...
def teachers_and_divisions_two
  ...
  execute(<<-SQL)
  SELECT teachers.name, CASE
  WHEN dept_id IN (1, 2) THEN 'Sci' WHEN dept_id = 3 THEN 'Art' ELSE 'None' END
  FROM teachers
  SQL
end
